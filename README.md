# 👋 Hi, I'm Alexandros Tsingos (Αλέξανδρος Τσίγγος)

🎓 **MSc in Data and Web Science (Final Semester)** – Aristotle University of Thessaloniki
💼 **Working on family company** – Taxi Van 9 Thessaloniki
🤖 **Experienced in:** Machine Learning, Explainable AI (XAI), LLM Applications, and RAG
📍 **Based in:** Thessaloniki, Greece
📬 [atsiggos@gmail.com](mailto:atsiggos@gmail.com) | [LinkedIn](https://www.linkedin.com/in/alexandros-tsingos/) | [GitHub](https://github.com/Alexisjr2001)

<br>

### 🚀 About Me

I am a Computer Science graduate currently in my final semester of the Data and Web Science MSc program. I combine a methodical, academic mindset with hands-on operational experience managing a family transportation and transfer service. My technical focus lies in bridging complex machine learning theory with practical, explainable applications. 

### 📚 Featured Projects
* **🔭 BSc Thesis – Domestic Electricity Forecasting:** Developed a forecasting system using Machine Learning techniques to predict electricity consumption patterns.


### ✨ Currently Focused On

* 🤖 Starting my master's thesis on Explainable AI (XAI).
* 🚗 Managing fleet logistics and daily operations at Taxi Van 9 Thessaloniki.

<br>

<div id="user-content-badges" align="center" dir="auto">
  <a href="https://www.linkedin.com/in/alexandros-tsingos/" rel="nofollow">
    <img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn Badge" style="max-width: 100%;">
  </a>
</div>
